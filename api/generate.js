...
// Glide body: { "profiel": "...", "naam": "Naam", "recruiternaam": "Recruiter" }

const OPENAI_URL = "https://api.openai.com/v1/chat/completions";
const MODEL = "gpt-4o-mini";
const TEMPERATURE = 0.3;

// ---- Helpers (NIET exporteren) ----
...
function safeJsonParse(s) {
  ...
  // Pak alleen het eerste complete JSON-blok { ... }
...
async function openaiChat({ system, user, temperature = TEMPERATURE, max_tokens = 1600 }) {
...
export default async function handler(req, res) {
  try {
    ...
    if (!step1Parsed.ok) {
      return res.status(500).json({ error: "JSON uit stap 1 niet te parsen", raw: step1Text });
    }
    ...
    if (!step2Parsed.ok) {
      return res.status(500).json({ error: "JSON uit stap 2 niet te parsen", raw: step2Text });
    }
    ...
    if (!step3Parsed.ok) {
      return res.status(500).json({ error: "JSON uit stap 3 niet te parsen", raw: step3Text });
    }
